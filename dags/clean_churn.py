...
def clean_churn_dataset():
    ...
    def extract():
        ...
        sql = """
            ...
            from users_churn
        """
        data = pd.read_sql(sql, conn).drop(columns=['id'], errors='ignore')
        conn.close()
        return data
    ...
    def transform(data: pd.DataFrame):
        def remove_duplicates(data: pd.DataFrame) -> pd.DataFrame:
            """Removes duplicates from the extracted dataset"""
        ...
        def remove_outliers(data: pd.DataFrame) -> pd.DataFrame:
            """Removes outliers using 1.5 margin"""
            ...
            return data[data[col].between(lower, upper)]
        
        def fill_missing_values(data: pd.DataFrame) -> pd.DataFrame:
            """Fills missing values with mean for num and mode for obj in the extracted dataset"""
            cols_with_nans = data.isnull().sum()
            cols_with_nans = cols_with_nans[cols_with_nans > 0].index
            cols_with_nans = cols_with_nans.drop('end_date', errors='ignore')
        ...
        data = fill_missing_values(remove_outliers(remove_duplicates(data))) # cringe
        return data
